/* Domoticz Button 
   ...
   TODO - 2020-12-20
     ...
     simplify display
       instead of displayVisible and alertvisible try
         emum screenMode_t { SM_SHOW, SM_BLANK, SM_ALERT }
...
void send_domoticz_cmd(int dev, int32_t value, bool isLevel = false) {
  char buffer[MSG_SZ];    
  if ( devices[dev].type == DT_SWITCH || devices[dev].type == DT_PUSH_OFF || (devices[dev].type == DT_DIMMER && !isLevel) ) 
...
bool displayVisible = true;        // is the state currently visible
...
enum editmode_t { 
   EM_STATUS,       // editing device On/Off status
   ...
   EM_DIM_LEVEL,    // showing dimmer level 
   ...
   EM_SELECTOR,     // showing selector choice 
...
void OnButtonRotated(int position) {
  if (editmode == EM_STATUS)
    cdev = position;
...
void displayDevice(uint16_t index, bool alert=false) {
  ...
  // build bottom row
  if (editmode == EM_DIM_LEVEL) {
      sprintf(llbuf, "< %d >", dimLevel * 10);
  } else if (editmode == EM_SELECTOR) {
      sprintf(llbuf, "< %s >", devicestatus[ selChoice + selectors[devices[index].xstatus].status0 ]);
  } else {  // (editmode == EM_DEVICES)
    if (devices[index].type == DT_DIMMER)
      sprintf(llbuf, "%s @ %d%%", devicestatus[devices[index].status], devices[index].xstatus * 10);
      ...
      sprintf(llbuf, "%s", devicestatus[ devices[index].status + selectors[devices[index].xstatus].status0 ]);
    } else {
      sprintf(llbuf, "%s", devicestatus[devices[index].status]);  
    }  
  } 
...
void setup() {
  ...
  display.drawString(64, TOP_ROW, "Connecting to");
  display.drawString(64, MIDDLE_ROW, "Wi-Fi...");
  display.display();

  // init rotary encoder
  rotary.setLimits(deviceCount-1);
  rotary.setPosition(0); // start at 
  rotary.OnButtonRotated(OnButtonRotated);
  ...
  display.clear();
  display.drawString(64, TOP_ROW, "Connected");
  display.drawString(64, MIDDLE_ROW, String("IP ") + WiFi.localIP().toString());
  display.drawString(64, BOTTOM_ROW, "Updating...");
  display.display();
